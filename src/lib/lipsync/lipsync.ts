...
export class Lipsync {
  ...
  public state: FSMStates = FSMStates.silence; // Changed to public to be accessible
  ...
  constructor(
    ...
    // Create audio context only if in browser
    if (typeof window !== "undefined") {
      ...
      // Dummy values for SSR
      this.audioContext = {} as AudioContext;
      this.analyser = {} as AnalyserNode;
      this.dataArray = new Uint8Array(0);
      this.sampleRate = 44100;
      this.binWidth = 44100 / 2048;
    }
    this.history = [];
  ...
  connectAudio(audio: HTMLMediaElement) {
    ...
    // Check if source already exists? No, createMediaElementSource can only be called once per element.
    // If re-using element, we might need to handle this.
    // However, the standard practice is to create a new source or reuse the graph.
    // For now, let's assume a new element or handle the error if already connected.
    try {
        ...
        console.log("Audio element might already be connected", e);
    }
  }
  ...
  async connectMicrophone() {
    try {
      ...
      source.connect(this.analyser);
      // Do not connect microphone to destination to avoid feedback loop if playing through speakers
      // this.analyser.connect(this.audioContext.destination); 
      return source;
    } catch (err) {
  ...
  detectState() {
    ...
    // Map viseme to state
    const newState = VISEMES_STATES[topViseme];
  ...
  processAudio() {
    if (this.isSimulated) {
      // Simulate lipsync by picking random vowel visemes periodically
      const vowels = [VISEMES.aa, VISEMES.E, VISEMES.I, VISEMES.O, VISEMES.U];
      const currentTime = performance.now();
      
      // Change viseme every 100-200ms
      if (currentTime - this.visemeStartTime > 100 + Math.random() * 100) {
        this.viseme = vowels[Math.floor(Math.random() * vowels.length)];
        this.visemeStartTime = currentTime;
      }
      return;
    }
